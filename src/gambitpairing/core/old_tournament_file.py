...
class Tournament:
    ...
    def create_pairings(
        ...
        floated_this_round: Set[str] = (
            set()
        )  # Track players floated *in this round's processing*

        for score in sorted_scores:
            current_score_group = sorted(
                score_groups[score], key=lambda p: (-p.rating, p.name)
            )
            ...
            )  # Re-sort combined group
            unpaired_from_higher_groups = []  # Clear for next iteration

            logging.debug(
                f"Processing Score Group: {score}, Players: {[p.name for p in group_to_pair]}"
            )
            ...
            if len(group_to_pair) % 2 == 1:
                ...
                float_candidates = [
                    p for p in group_to_pair if p.id not in floated_this_round
                ]
                if (
                    not float_candidates
                ):  # All have floated this round (should not happen if logic is correct)
                    logging.warning(
                        f"All players in group for score {score} already floated this round. Taking from original group_to_pair."
                    )
                ...
                if float_candidates:  # Ensure there's someone to float
                    ...
                    float_candidates.sort(
                        key=lambda p: (
                            p.float_history[-1] if p.float_history else -999,
                            p.rating,
                            p.name,
                        )
                    )
                    ...
                    unpaired_from_higher_groups.append(
                        floater
                    )  # This floater moves to the next (lower) score group
                    floated_this_round.add(floater.id)
                    floater.float_history.append(current_round)
                    group_to_pair.remove(floater)
                    logging.info(
                        f"Player {floater.name} floated down from score group {score}."
                    )
                else:
                    logging.warning(
                        f"Odd group for score {score} but no float candidates found. This is unusual."
                    )
            ...
            while len(temp_unpaired_in_group) >= 2:
                ...
                for idx, p2_candidate in enumerate(temp_unpaired_in_group):
                    ...
                    if (
                        p1_pref is not None
                        and p2_cand_pref is not None
                        and p1_pref == p2_cand_pref
                    ):
                    ...
                    if current_conflict_score < min_color_conflict_score:
                        ...
                        if (
                            min_color_conflict_score == 0
                        ):  # Perfect color match (or no preference from one/both)
                            break
                ...
                if best_opponent_for_p1 and best_opponent_idx != -1:
                    ...
                    if pref1 == W and (pref2 == B or pref2 is None):
                        white, black = p1, p2
                    elif pref1 == B and (pref2 == W or pref2 is None):
                        white, black = p2, p1
                    elif pref2 == W and (pref1 == B or pref1 is None):
                        white, black = p2, p1  # Redundant if covered by above
                    elif pref2 == B and (pref1 == W or pref1 is None):
                        white, black = p1, p2  # Redundant
                    ...
                    if white is None:  # If not assigned yet
                        if pref1 == W:
                            white, black = (
                                p1,
                                p2,
                            )  # p1 gets preference if p2 has same or no conflicting
                        elif pref1 == B:
                            white, black = p2, p1
                        elif pref2 == W:
                            white, black = p2, p1
                            ...
                            if p1_bal > p2_bal:
                                white, black = p2, p1  # p1 more W, gets B
                            elif p2_bal > p1_bal:
                                white, black = p1, p2  # p2 more W, gets B
                            else:
                                white, black = (
                                    (p1, p2) if p1.rating >= p2.rating else (p2, p1)
                                )
                    ...
                    logging.warning(
                        f"Player {p1.name} could not be paired in score group {score} and will be carried down."
                    )
        ...
        final_unpaired_list.sort(
            key=lambda p: (-p.rating, p.name)
        )  # Sort them for consistent processing
        ...
        if len(final_unpaired_list) % 2 == 1:
            ...
            bye_candidate_player = final_unpaired_list[
                -1
            ]  # Typically lowest rated of this small group
            bye_player = self._get_eligible_bye_player(
                [bye_candidate_player]
            )  # Pass as list
            if bye_player:
                if bye_player in final_unpaired_list:
                    final_unpaired_list.remove(bye_player)
                logging.info(
                    f"Round {current_round} Bye assigned to: {bye_player.name}"
                )
            else:
                ...
                logging.error(
                    f"Critical: Odd player {bye_candidate_player.name} remains but cannot be assigned a bye. Pairing may be incomplete."
                )
        ...
        while len(temp_final_unpaired) >= 2:
            ...
            for idx, p2_candidate in enumerate(temp_final_unpaired):
                if frozenset({p1.id, p2_candidate.id}) not in self.previous_matches:
                    ...
                    white, black = None, None
                    if pref1 == W and (pref2 == B or pref2 is None):
                        white, black = p1, p2_candidate
                    elif pref1 == B and (pref2 == W or pref2 is None):
                        white, black = p2_candidate, p1
                    elif pref2 == W and (pref1 == B or pref1 is None):
                        white, black = p2_candidate, p1
                    elif pref2 == B and (pref1 == W or pref1 is None):
                        white, black = p1, p2_candidate
                    if white is None:
                        ...
                        if p1_bal > p2_bal:
                            ...
                            white, black = (
                                (p1, p2_candidate)
                                if p1.rating >= p2_candidate.rating
                                else (p2_candidate, p1)
                            )
                    ...
                    if allow_repeat_pairing_callback is not None:
                        ...
                        if proceed:
                            ...
                            if white is None:
                                ...
                                p2_vc = [
                                    c
                                    for c in p2_candidate.color_history
                                    if c is not None
                                ]
                                p1_bal = p1_vc.count(W) - p1_vc.count(B)
                                p2_bal = p2_vc.count(W) - p2_vc.count(B)
            ...
            if not paired_p1:
                logging.error(
                    f"Player {p1.name} could not be paired in the final pairing stage."
                )
